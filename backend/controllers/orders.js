...
const getOrders = async (req, res) => {
  try {
    ...
    res.json(orders.map(o => {
      let items = o.items;
      // MySQL can return this already parsed or as a string depending on version
      if (typeof items === 'string') {
        try { items = JSON.parse(items); } catch { items = []; }
      }
      // Filter out null stub rows from LEFT JOIN when no items exist
      if (Array.isArray(items)) items = items.filter(i => i && i.id !== null);
